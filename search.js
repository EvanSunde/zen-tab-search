...
(function(root) {
  ...
  function prepare(item) {return {...item, search:normalize([item.title,item.url,item.subtitle,item.folder,item.workspaceName].filter(Boolean).join(" "))};}
  function search(items, query, scope=null) {
    const tokens=normalize(query).trim().split(/\s+/).filter(Boolean);
    return items.filter(i=>(!scope || (i.kind==='tabs' && i.windowId===scope.windowId && (i.workspaceId===scope.id || i.essential))) && tokens.every(t=>tokenMatch(t,i.search)))
      .sort((a,b)=>(b.lastUsed||0)-(a.lastUsed||0) || a.title.localeCompare(b.title) || String(a.id).localeCompare(String(b.id)));
  }
  root.ZenSearch={normalize,prepare,search,tokenMatch};
